...
from toontown.uberdog.PartyAvatarAI import PartyAvatarAI
...
class DistributedPartyManagerAI(DistributedObjectAI):
    ...
    def addPartyRequest(self, hostId, startTime, endTime, isPrivate, inviteTheme, activities, decorations, inviteeIds):
        """Add a new party."""
        self.notify.debug("addPartyRequest")
        ...
        if not toonSender:
            ...
            self.notify.debug('addPartyRequest toon %d is not in our district' % senderId)
            return
        ...
        if validPartyRequest:
            ...
            self.notify.debug('inValid party because : %s' % costOfPartyOrError)
    ...
    def validatePartyAndReturnCost(self, hostId, startTime, endTime, isPrivate, inviteTheme, activities, decorations, inviteeIds):
        self.notify.debug("validatePartyAndReturnCost")
        ...
        for activityTuple in activities:
            ...
            if activityId in PartyGlobals.VictoryPartyActivityIds:
                if not simbase.air.holidayManager.isHolidayRunning(ToontownGlobals.VICTORY_PARTY_HOLIDAY):
                    return (False, "Can't add activity %s not during Victory Party " % activityId)
            ...
            if activityId in PartyGlobals.WinterPartyActivityIds:
                if not simbase.air.holidayManager.isHolidayRunning(ToontownGlobals.WINTER_DECORATIONS) or not simbase.air.holidayManager.isHolidayRunning(ToontownGlobals.WACKY_WINTER_DECORATIONS):
                    return (False, "Can't add activity %s not during Winter Party " % activityId)
            if activityId in PartyGlobals.WinterPartyReplacementActivityIds:
                if simbase.air.holidayManager.isHolidayRunning(ToontownGlobals.WINTER_DECORATIONS) or simbase.air.holidayManager.isHolidayRunning(ToontownGlobals.WACKY_WINTER_DECORATIONS):
                    return (False, "Can't add activity %s not during Winter Party " % activityId)

            # Check for valentines
            if activityId in PartyGlobals.ValentinePartyActivityIds:
                if not simbase.air.holidayManager.isHolidayRunning(ToontownGlobals.VALENTINES_DAY):
                    return (False, "Can't add activity %s during Valentines Party " % activityId)
            if activityId in PartyGlobals.ValentinePartyReplacementActivityIds:
    ...
    def respondToInviteFromMailbox(self, context, inviteKey, newStatus, mailboxDoId):
        ...
        self.notify.debug("respondToInvite")
        senderId = self.air.getAvatarIdFromSender()
        toonSender = simbase.air.doId2do.get(senderId)
        if not toonSender:
            # the toon is not on our district, let the party manager on that district handle it
            self.notify.debug('respondToInviteFromMailbox toon %d is not in our district' % senderId)
            return
        self.air.sendUpdateToDoId("DistributedPartyManager",
                                  'respondToInvite',
                                  OtpDoGlobals.OTP_DO_ID_TOONTOWN_PARTY_MANAGER,
    ...
    def respondToInviteResponse(self, mailboxDoId, context, inviteKey,  retcode, newStatus):
        ...
        self.notify.debug("respondToInviteResponse")
    ...
    def changePrivateRequest(self, partyId, newPrivateStatus):
        ...
        if not toonSender:
            # the toon is not on our district, let the party manager on that district handle it
            self.notify.debug('changePrivateRequest toon %d is not in our district' % senderId)
            return
    ...
    def changePartyStatusRequest(self, partyId, newPartyStatus):
        ...
        if not toonSender:
            # the toon is not on our district, let the party manager on that district handle it
            self.notify.debug('changePartyStatusRequest toon %d not in our district' % senderId)
            return
        errorCode = self.partyFieldChangeValidate(partyId)
        if errorCode != PartyGlobals.ChangePartyFieldErrorCode.AllOk:
            # immediately say we have an error then return
    ...
    def getAllPublicParties(self):
        ...
        self.notify.debug("getAllPublicParties : %s" % returnParties)
    ...
    def updateToPublicPartyCountUdToAllAi(self, hostId, newCount):
        ...
        """
        self.notify.debug("updateToPublicPartyCountUdToAllAi : hostId=%s newCount=%s" % (hostId, newCount))
        if hostId in self.hostAvIdToAllPartiesInfo:
    ...
    def updateToPublicPartyInfoUdToAllAi(self, hostId, time, shardId, zoneId, isPrivate, numberOfGuests, hostName, activityIds, partyId):
        ...
        """
        self.notify.debug("updateToPublicPartyInfoUdToAllAi : hostId=%s time=%s shardId=%s zoneId=%s isPrivate=%s numberOfGuests=%s hostName=%s" %
    ...
    def delete(self):
        self.notify.debug("BASE: delete: deleting DistributedPartyManagerAI object")
        self.ignoreAll()
    ...
    def getPartyZone(self, hostId, zoneId, planningParty):
        self.notify.debug("getPartyZone: hostId=%s zoneId=%s planningParty=%s" % (hostId, zoneId, planningParty))
        ...
        if planningParty:
            ...
            self.notify.debug("getPartyZone : Avatar %s is planning party in zone %s" % (senderId, zoneId))
        ...
        if senderPartyZoneId is not None:
            ...
            try:
                if senderPartyZoneId == avPartyZoneId and not isHostStartingParty:
                    self.notify.debug("We are staying in the same zone %s, don't delete." % senderPartyZoneId)
                    self.__sendPartyZoneToClient(senderId, hostId)
                    return
                else:
                    self.notify.debug("Sender is goint to a different party. Party av zone = %s, sender zone = %s." % (avPartyZoneId, senderPartyZoneId))
            except:
                self.notify.debug("Sender is not teleporting to the same party.")
    ...
    def announceToonEnterPartyZone(self, avId, hostId, zoneId):
        ...
        """
        self.notify.debug('announceToonEnterPartyZone: %s %s %s' % (avId, hostId, zoneId))
    ...
    def announceToonExitPartyZone(self, avId, hostId, zoneId):
        ...
        a party """
        self.notify.debug('announceToonExitPartyZone: %s %s %s' %
                          (avId, hostId, zoneId))
    ...
    def __addReferences(self, senderId, hostId):
        self.notify.debug("__addReferences : senderId = %s  hostId = %s" % (senderId, hostId))
    ...
    def __removeReferences(self, avId, zoneId):
        try:
            ...
            self.notify.debug("we weren't in the zoneIdToGuestAvIds for %s." % zoneId)
            pass
    ...
    def setPartyZoneId(self, avId, zoneId):
        ...
        self.notify.debug("%s(%s):Added %s:%s" % (defName, lineno, avId, zoneId))
    ...
    def clearPartyZoneId(self, avId, zoneIdFromClient=None):
        ...
        if removeFromDict:
            self.notify.debug("%s(%s):Removed %s:%s" % (defName, lineno, avId, self.avIdToPartyZoneId[avId]))
            del self.avIdToPartyZoneId[avId]

    def handleGetPartyInfo(self, partyInfo, inviteeIds):
        self.notify.debug("handleGetPartyInfo for host %s" % partyInfo.hostId)
        ...
        if partyInfo.hostId not in self.avIdToPartyZoneId:
            self.notify.warning("Party Zone info was requested, but the guest left before it could be recieved: %d" % partyInfo.hostId)
            return
    ...
    def exitParty(self, zoneId):
        senderId = self.air.getAvatarIdFromSender()
        self.notify.debug("exitParty(%s)" % senderId)
    ...
    def __handleUnexpectedExit(self, avId):
        ...
        if avId in self.avIdToPartyZoneId:
            ...
            self.notify.debug("unexpected exit and %s is not in avIdToPartyZoneId" % avId)
        return None

    def __exitParty(self, avId, zoneIdFromClient=None):
        self.notify.debug("__exitParty(%d)" % avId)
        self.notify.info("__exitParty(%d)" % avId)
        ...
        if avZoneId is not None:
            ...
            if avZoneId in self.zoneIdToGuestAvIds:
                if avId in self.zoneIdToGuestAvIds[avZoneId]:
                    ...
                    self.notify.debug("wasn't in zoneIdToGuestAvIds list: %s, %s" % (avZoneId, avId))
            else:
                self.notify.debug("wasn't in zoneIdToGuestAvIds: %s, %s" % (avZoneId, avId))
            ...
            self.notify.debug("__exitParty can't find zone for %d" % avId)
    ...
    def __cleanupParty(self, hostId, zoneId):
        self.notify.debug("__cleanupParty hostId = %d, zoneId = %d" % (hostId, zoneId))
    ...
    def __deleteParty(self, hostId):
        ...
        self.notify.debug("__deleteParty(hostId=%s)" % hostId)
        ...
        if hostId in self.hostAvIdToPartiesRunning:
            if self.hostAvIdToPartiesRunning[hostId] != None:
                ...
                self.notify.debug('DistributedPartyAI requestDelete, doId=%d' % getattr(self.hostAvIdToPartiesRunning[hostId], 'doId'))
    ...
    def __bootGuests(self, hostId, zoneId):
        self.notify.debug("__bootGuests (hostId=%s  zoneId=%s)" % (hostId, zoneId))
        try:
    ...
    def __setPartyEnded(self, hostId, zoneId):
        ...
        """
        self.notify.debug("__setPartyEnded (hostId=%s  zoneId=%s)" % (hostId, zoneId))
    ...
    def deductMoneyFromOfflineToon(self, toonId, cost):
        ...
        def handleAvatar(dclass, fields):
            if dclass != self.air.dclassesByName['DistributedToonAI']:
                self.notify.warning("deductMoneyFromOfflineToon - toon %s returned a non-toon. buying a party for free!cost=%s" % (toonId, cost))
                self.air.writeServerEvent('suspicious', toonId, "deductMoneyFromOfflineToon - toon %s returned a non-toon. buying a party for free!cost=%s" % (toonId, cost))
                return

            def __avatarOnlineResp(checkedAvId, activated):
                newFields = fields

                avatarFields = self.copyDict(newFields, 'setMaxMoney', 'setMaxBankMoney', 'setMoney', 'setBankMoney')

                toon = PartyAvatarAI.createFromFields(avatarFields)
                ...
                if toon:
                    ...
                    if result:
                        ...
                        success = self.gotToonToTakeMoney(toon, checkedAvId, fields, activated)

                        if success:
                            self.notify.info("deductMoneyFromOfflineToon - deducting %s from offline toon %s newTotalMoney=%s" % (cost, toonId, newTotalMoney))
                            return
                        else:
                            self.notify.warning("deductMoneyFromOfflineToon - Host %s got away with buying a party he can't afford! totalMoney=%s cost=%s" % (toonId, totalMoney, cost))
                            self.air.writeServerEvent('suspicious', toonId, "deductMoneyFromOfflineToon - Host %s got away with buying a party he can't afford! totalMoney=%s cost=%s" % (toonId, totalMoney, cost))
                            return
                    else:
                        self.notify.warning("deductMoneyFromOfflineToon - Host %s got away with buying a party he can't afford! totalMoney=%s cost=%s" % (toonId, totalMoney, cost))
                        self.air.writeServerEvent('suspicious', toonId, "deductMoneyFromOfflineToon - Host %s got away with buying a party he can't afford! totalMoney=%s cost=%s" % (toonId, totalMoney, cost))
                    ...
                    self.notify.warning("deductMoneyFromOfflineToon - toon %s not found. buying a party for free!cost=%s" % (toonId, cost))
                    self.air.writeServerEvent('suspicious', toonId, "deductMoneyFromOfflineToon - toon %s not found. buying a party for free!cost=%s"  % (toonId, cost))
                    return

            self.air.getActivated(toonId, __avatarOnlineResp)
        self.air.dbInterface.queryObject(self.air.dbId, toonId, handleAvatar)

    def gotToonToTakeMoney(self, toon, checkedAvId, fields, activated):
        self.ableToTakeMoney = True

        newMoney = toon.getMoney()
        newBankMoney = min(toon.getBankMoney(), toon.getMaxBankMoney())

        if activated:
            self.air.sendUpdateToDoId("DistributedToon", "setMoney", checkedAvId, [newMoney])
            self.air.sendUpdateToDoId("DistributedToon", "setBankMoney", checkedAvId, [newBankMoney])
        else:
            def __handleSetMoney(fields):
                if fields:
                    self.ableToTakeMoney = False

            self.air.dbInterface.updateObject(self.air.dbId,
                                              checkedAvId,
                                              self.air.dclassesByName['DistributedToonAI'],
                                              {'setMoney': (newMoney,),
                                               'setBankMoney': (newBankMoney,)},
                                              {'setMoney': (fields['setMoney'],),
                                               'setBankMoney': (fields['setBankMoney'],)},
                                              __handleSetMoney)

        return self.ableToTakeMoney
