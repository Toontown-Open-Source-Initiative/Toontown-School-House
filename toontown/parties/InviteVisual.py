...
# Created: Sep 2008
#
# Purpose: The visual rendering of a party invite with activities and time.
#
# -------------------------------------------------------------------------------
...
class InviteVisual(DirectFrame):
    """
    This class provides the visual for a party invitation
    """
    ...
    def __init__(self, parent):
        ...
        self.inviteThemesIdToInfo = {
            PartyGlobals.InviteTheme.Birthday: (self.gui.find("**/birthdayPage"), TTLocalizer.PartyPlannerBirthdayTheme, (0.0, 0.0, 0.0, 1.0)),
        ...
        self.inviteThemeBackground = DirectFrame(
            parent=self,
            image=self.inviteThemesIdToInfo[0][0],
            relief=None,
        )
        ...
        # If this is being shown for someone with no friends or someone who
    ...
    def updateInvitation(self, hostsName, partyInfo):
        ...
        if self.noFriends:
            self.whenTextLabel["text"] = TTLocalizer.PartyPlannerInvitationWhenSentenceNoFriends % (
                PartyUtils.formatDate(self.partyInfo.startTime.year, self.partyInfo.startTime.month, self.partyInfo.startTime.day),
                PartyUtils.formatTime(self.partyInfo.startTime.hour, self.partyInfo.startTime.minute),
            )
        else:
            self.whenTextLabel["text"] = TTLocalizer.PartyPlannerInvitationWhenSentence % (
                PartyUtils.formatDate(self.partyInfo.startTime.year, self.partyInfo.startTime.month, self.partyInfo.startTime.day),
                PartyUtils.formatTime(self.partyInfo.startTime.hour, self.partyInfo.startTime.minute),
            )
