...
def __doSmooch(attack, hp = 0):
    ...
    effectTrack = Parallel(Sequence(ActorInterval(toon, 'smooch')), lipstickTrack, __getSoundTrack(level, 2, node=toon))
    for target in targets:
        ...
        delay = tThrow + dThrow
        mtrack = Parallel(lipsTrack, Sequence(Wait(delay), ActorInterval(target, 'conked')), Sequence(Wait(delay), Func(__healToon, target, hp)))
        effectTrack.append(mtrack)
