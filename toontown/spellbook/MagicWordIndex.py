...
class MaxToon(MagicWord):
    ...
    def handleWord(self, invoker, avId, toon, *args):
        ...
        toon.b_setCogParts([
            ...
            CogDisguiseGlobals.PartsPerSuitBitmasks[5]
        ])
        toon.b_setCogLevels([ToontownGlobals.MaxCogSuitLevel] * 6 + [0])
        toon.b_setCogTypes([7] * 6 + [0])

        toon.b_setCogCount(list(CogPageGlobals.COG_QUOTAS[1]) * 6)
        cogStatus = [CogPageGlobals.COG_COMPLETE2] * SuitDNA.suitsPerDept
        toon.b_setCogStatus(cogStatus * 6)
        toon.b_setCogRadar([1] * 6)
        toon.b_setBuildingRadar([1] * 6)
