...
from pymysql.connections import Connection as DirectMySQLdb
...
class TTCRDBConnection:
    ...
    def enterInitializing(self):
        # create database
        cursor = self.getCursor()
        initDb = ConfigVariableBool('want-code-redemption-init-db', False).getValue()
...
class TTCodeRedemptionDB(DirectObject):
    ...
    DoSelfTest = ConfigVariableBool('code-redemption-self-test', True).getValue()
    ...
    def __init__(self, air, host, port, user, passwd, dbname):
        ...
        if ConfigVariableBool('code-redemption-subprocess-test', False).getValue():
            self._testSubProc()
