...
class ToonTipPanel(DirectFrame):
    ...
    def __init__(self):
        ...
        self.frameTitle = DirectLabel(pos=(-0.115, 0.5, 0.15), scale=(0.08, 1.0, 0.15), sortOrder=55, text_font=ToontownGlobals.getSignFont(), text='', text_fg=(0.0, 0.55, 1.0, 1.0), relief=None, text_align=TextNode.ACenter)
    ...
    def changeTipText(self, num):
        if num in TTLocalizer.ToonTipByNum.keys():
            self.frameText['text'] = TTLocalizer.ToonTipByNum[num][0]
            self.frameText['text_align'] = TextNode.ALeft
            self.frameTitle['text'] = TTLocalizer.ToonTipByNum[num][1]
